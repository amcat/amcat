...
"""
Preprocess Script for adding/removing Amcat articles to Solr from the model
"""
...
class Solr(AnalysisScript):
    ...
    def __init__(self, analysis):
        super(Solr, self).__init__(analysis, triples=False, tokens=False)

    def add_articles(self, article_ids):
        dicts = list(create_article_dicts(article_ids))
    ...
    def delete_articles(self, article_ids):
        log.debug("Removing %i articles from solr" % len(article_ids))
        s = solr.SolrConnection(b'http://localhost:8983/solr')
        s.delete_many(article_ids)
        s.commit()
...
def create_article_dicts(article_ids):
