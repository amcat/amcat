...
class TestAmcatES(amcattest.AmCATTestCase):
    def setup(self):
        ...
        Article.create_articles([a, b, c, d], articleset=s1, create_id=True)
        ES().flush()
    ...
    def test_list_media(self):
        ...
        s1 = amcattest.create_test_set()
        Article.create_articles(arts[:5], articleset=s1, create_id=True)
        ES().flush()
        self.assertEqual(set(s1.get_mediums()), set(media[:5]))

        s2 = amcattest.create_test_set(project=s1.project)
        Article.create_articles(arts[5:], articleset=s2, create_id=True)
        ES().flush()
    ...
    def test_query_all(self):
        ...
        Article.create_articles(arts, articleset=s, create_id=True)
        ES().flush()
