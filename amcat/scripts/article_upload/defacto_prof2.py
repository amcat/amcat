...
def get_article(e):
    ...
    print ">>>>", medium, date, page
    headline = get_headline(e)
    print ">>>>>>>>>", headline
    body = get_body(e)
    section = get_section(e)
    medium = get_or_create(Medium, name=medium)
    print "@@@@@", dict(headline=headline, text=body, date=date, pagenr=page, section=section, medium=medium)
...
def get_relevant_para(elements):
    # elements are para - table - *para*, or just para+
    if elements[1].tag == 'paragraph-definition':
        return elements[0]
    else:
        return elements[2]
...
def split_xml(xml):
    ...
    yield get_relevant_para(breaks[0].xpath("preceding-sibling::*"))
    return
...
HEADLINE_XPATHS = ["descendant-or-self::paragraph-definition[@style-number='s0004']//inline[@bold='true']",
                   "descendant-or-self::paragraph-definition[@style-number='s0001']//inline"]

def get_headline(e):
    hl = apply_xpaths(e, HEADLINE_XPATHS)
    for x in hl:
        if x.text.strip():
            return x.text.strip()
...
def apply_xpaths(e, xpaths):
    for xpath in xpaths:
        result = e.xpath(xpath)
        if result:
            return result
    raise ValueError("Cannot parse %r using %r" % (e, xpaths))
    
SECTION_XPATHS = ["descendant-or-self::paragraph-definition[@style-number='s0004']//inline[@italics='true']",
                  "descendant-or-self::paragraph-definition[@style-number='s0001']//inline[@italics='true']"]
                  
def get_section(e):
    pars = apply_xpaths(e, SECTION_XPATHS)
    for par in pars:
        try:
...
META_PAR_XPATHS = ["descendant-or-self::paragraph-definition[@style-number='s0004']//inline[@italics='true']",
                  "descendant-or-self::paragraph-definition[@style-number='s0001']//inline[@italics='true']",
                  "descendant-or-self::paragraph-definition[@style-number='s0001']//para",
                  ]

def get_meta(e):
    pars = apply_xpaths(e, META_PAR_XPATHS)
...
def get_xml(bytes):
    with NamedTemporaryFile() as f:
        cc = open('/tmp/test_recoded.rtf', 'w')

        bytes = bytes.replace('_'*67, '\\page')
        for b in bytes:
            if ord(b) >= 128:
                b = b.decode('latin-1').encode('rtfunicode')[:-1] # strip off substition char - by now writing the escape directly is probably easier than unsing rtfunicode...
            f.write(b)
            cc.write(b)
