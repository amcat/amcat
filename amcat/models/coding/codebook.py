...
class TestCodebook(amcattest.AmCATTestCase):
    ...
    def test_get_aggregation_mapping(self):
        ...
        D = Codebook.objects.get(id=D.id)
        D.cache_labels(language)

        with self.checkMaxQueries(0, "Get aggregation mapping with cached labels"):
            D.get_aggregation_mapping(language)
