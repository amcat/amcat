...
class SPSSRow(object):
    def __init__(self, spssfile, row):
        self.spssfile = spssfile
        self.row = row
    def get(self, varname):
        return self.spssfile.get(varname, self.row)

class SPSSFile(object):
    def __init__(self, file, skipdata = False):
        self.variables = []
        self.vardict = {} 
        self.data = []
    ...
    def addvar(self, var):
        ...
        self.vardict[var.name.lower()] = var
    def getvar(self, varname, default=Exception):
        result = self.vardict.get(varname.lower(), default)
        if result == Exception:
            raise KeyError(varname.lower())
        return result
    def get(self, var, row):
        if type(var) in (str, unicode):
            var = self.vardict[var.lower()]
        return row[var.index]
    def rows(self):
        for row in self.data:
            yield SPSSRow(self, row)
    def init(self, file):
        r = PorReader(file)
        r.skip(5)
        h = r.consumeOne()
...
if __name__ == '__main__':
    import sys
    fn = (sys.argv + ['/data/tmp/test5.por'])[1]
    f = SPSSFile(fn, skipdata=True)
