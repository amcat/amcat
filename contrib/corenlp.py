...
# Cleaned up and modified version of corenlp-python from
#    https://github.com/dasmith/stanford-corenlp-python

"""
python corenlp.py -S [OPTIONS]       # server mode
python corenlp.py [OPTIONS] < text   # client mode

Runs the Stanford CoreNLP suite python json-rpc server or client
"""
...
class StanfordCoreNLP(object):
    ...
    """
    def __init__(self, corenlp_path=".", corenlp_version="2012-07-09", models_version=None):
        """
        ...
        # spawn the server
        jarstr = ":".join(jars)
        cmd = "{java_path} -Xmx1800m -cp {classpath} {classname}".format(**locals())
        
        print cmd
        self._server = pexpect.spawn(cmd)
        
        print "Starting the Stanford Core NLP parser."
        print " ..  Load pos tagger model (~5sec)"
        self._server.expect("done.", timeout=600)
        print " ..  Load NER-all classifier (~33sec)"
        self._server.expect("done.", timeout=600)
        print " ..  Load NER-muc classifier (~60sec)"
        self._server.expect("done.", timeout=600)
        print " ..  Load CoNLL classifier (~50sec)"
        self._server.expect("done.", timeout=200)
        print " ..  Loading PCFG (~3sec)"
        self._server.expect("done.", timeout=200)
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '-S', '--server', action="store_true", dest="serve",
        help='Run as server (if not given, run client request)'
        )
    parser.add_option(
        '-c', '--corenlp_path', dest="corenlp_path",
        help='Path to CoreNLP folder or jar file'
        )
    parser.add_option(
        '-v', '--corenlp_version', dest="corenlp_version",
        help='Version (ie 2012-01-01) of the corenlp jar'
        )
    parser.add_option(
        '-m', '--models_version', dest="models_version",
        help='Version (ie 2012-01-01) of the models jar'
        )
    options, args = parser.parse_args()

    if options.serve:
        kargs = {k: getattr(options, k) for k in ("corenlp_path", "corenlp_version", "models_version")
                 if getattr(options, k) is not None}
        print kargs
        serve(options.host, options.port, **kargs)
    else:
        print >>sys.stderr, "[Reading text from standard input...]"
        text = sys.stdin.read()
        sentences, coref = parse_remote(text, options.host, options.port)
