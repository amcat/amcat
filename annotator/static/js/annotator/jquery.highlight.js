 ...
 *
 * Changes Martijn Bastiaan: added lenient parameter. (From easyMark.)
 *
 */
 ...
 *******************/
function createAccentRegexp(str) {
    // Matches anything *but* a whitespace and replaces it.
    return deaccent(str).replace(/\S/g, function(c){ return charToAccentedCharClassMap[c] || c; });
}
...
jQuery.fn.highlight = function (words, options) {
    ...
    // Must be case insensitive if lenient
    settings.caseSensitive = (settings.lenient) ? false : settings.caseSensitive;
    ...
    // Replace all accented characters if lenient.
    if (settings.lenient){
        words = $.map(words, createAccentRegexp);
    }
